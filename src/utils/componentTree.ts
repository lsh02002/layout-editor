...
export const findComponentLocation = (
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    if (item.id === id) return { parentId, index };
    if (item.type === "container") {
...
export const removeComponentRecursive = (
  ...
  if (directIndex >= 0) {
    ...
    return { items: normalizeOrder(next), removed };
  }
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    if (result.removed) {
      const next = [...items];
      next[index] = { ...item, children: result.items };
      return { items: normalizeOrder(next), removed: result.removed };
    }
  }
...
export const insertComponentRecursive = (
  ...
  return items.map((item) => {
    if (item.type === "container" && item.id === parentId) {
      ...
      return { ...item, children: normalizeOrder(children) };
    }
    if (item.type === "container") {
      return {
        ...item,
...
export const cloneComponent = (component: LayoutComponent): LayoutComponent => {
  ...
  const assignIds = (item: LayoutComponent): LayoutComponent => {
    const next = { ...item, id: crypto.randomUUID() } as LayoutComponent;
...
export const containsComponent = (
  ...
  return component.type === "container"
    ? component.children.some((child) => containsComponent(child, targetId))
    : false;
};
...
export const updateLayoutRecursive = (
  ...
  items.map((item) => {
    if (item.id === id) {
      return { ...item, layout: { ...item.layout, ...newLayout } };
    }
    if (item.type === "container") {
      return {
        ...item,
